...
public class GradeCalculator {
    ...
    private double calculateFinalGrade(double[] grades) {
        if(grades.length < 2)
        throw new IllegalArgumentException("There need to be at least two grades entered.");
        ...
        double sum = 0;
        for (int i = 0; i < grades.length; i++) {
            if (i != smallestIndex && grades[i] >=0 && grades[i] <=4) {
                sum = sum + grades[i];
            }
            else
                throw new IllegalArgumentException("Grades cannot be greater than 4.0 or less than 0.0");
        }
